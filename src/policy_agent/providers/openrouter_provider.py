...
class OpenRouterProvider(BasePolicyProvider):
    """
    OpenRouter-backed synthesis provider.
    Connects to OpenRouter's OpenAI-compatible endpoint (supporting free open-source models).
    ...
    def __init__(self):
        try:
            ...
            raise ImportError("openai package is required for OpenRouterProvider. Run 'pip install openai'.")
    ...
    def generate_response(
        ...
        try:
            ...
            logger.error(f"OpenRouter API call failed: {e}. Falling back to deterministic output.")
